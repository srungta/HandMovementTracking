...
# Control parameters
FLIP_ORIGINAL_IMAGE = True
SHOW_MASK = False
SHOW_MASK_IN_COLOR = False
...
while True:
	...
	# Show the images
	if FLIP_ORIGINAL_IMAGE:
		opencv.imshow("Image captured from video with trace line.",
					  opencv.flip(image_from_video, 1))
	else:
		opencv.imshow(
			"Image captured from video with trace line.", image_from_video)
	if SHOW_MASK:
		opencv.imshow("Mask of the tracing point.", mask)
	if SHOW_MASK_IN_COLOR:
		opencv.imshow("Snippet from the original image being traced.", colored_mask)
